...
describe("route manifest", () => {
  ...
  it("partitions cleanly into the expected shape counts", () => {
    ...
    expect(count("owner-api")).toBe(29);
    expect(count("owner-page")).toBe(9);
    ...
    expect(ROUTE_MANIFEST.length).toBe(60);
  });

  it("exposes the shape helpers matching the partition", () => {
    expect(ownerApiRoutes().length).toBe(29);
    expect(ownerPageRoutes().length).toBe(9);
