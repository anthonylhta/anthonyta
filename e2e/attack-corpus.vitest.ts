...
describe("route manifest", () => {
  ...
  it("partitions cleanly into the expected shape counts", () => {
    ...
    expect(count("owner-api")).toBe(16);
    expect(count("owner-page")).toBe(6);
    ...
    expect(ROUTE_MANIFEST.length).toBe(44);
  });

  it("exposes the shape helpers matching the partition", () => {
    expect(ownerApiRoutes().length).toBe(16);
    expect(ownerPageRoutes().length).toBe(6);
