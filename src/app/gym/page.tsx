...
// The training log — owner-only, and every number on it decrypts in the browser.
export const dynamic = "force-dynamic";

export default async function GymPage() {
  // Owner-only: guests get a 404 (ADR 0022). The whole log lives inside the E2EE
  // gym envelope and decrypts in the client island — the server renders only the
  // shell, so there is nothing here to read past the gate.
    ...
    <main className="mx-auto flex min-h-dvh max-w-3xl flex-col px-4 py-6 sm:px-6">
      <div className="border border-hairline bg-surface/20">
        ...
        <div className="flex items-center justify-between border-b border-hairline px-4 py-2 text-xs">
          ...
          <span className="uppercase tracking-[0.2em] text-muted">gym</span>
          <span className="rounded border border-hairline px-1.5 py-0.5 text-[10px] text-amber">
            private
          </span>
        </div>

        <GymLog offline={!r2Enabled()} />
      </div>
