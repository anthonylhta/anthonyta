...
/**
 * The owner's static ECDH keypair for the sealed-box channel: the public point in
 * the clear (the cron seals to it) and the private key sealed under the MK, so it
 * sits beside the data yet only opens with the passphrase. `alg` is pinned so a
 * future curve swap is an explicit format migration, not a silent reinterpretation.
 */
export interface Snapkey {
  v: 1;
  alg: "ECDH-P256";
  pub_b64: string;
...
/** Strict shape guard for a stored Snapkey (server PUT gate + client parse). */
export function isSnapkey(x: unknown): x is Snapkey {
  if (typeof x !== "object" || x === null) return false;
  const k = x as Record<string, unknown>;
  return (
    k.v === 1 &&
 ...
 * here: the public point as raw bytes (65B, shared in the clear) and the private key
 * as PKCS#8 (to be sealed under the MK before storage). The caller discards both
 * handles after export and re-imports the private half non-extractable.
 */
 ...
 * side — the write-only channel the nightly cron uses to seal snapshots it can never
 * reopen. A fresh ephemeral keypair per call means the same plaintext never yields
 * the same envelope, and the ephemeral private half is dropped once the key derives.
 */
