...
describe("isValidVaultPath", () => {
  it("accepts the index, the search index, the manifest, and both envelope shapes", () => {
    ...
    expect(isValidVaultPath(VAULT_MANIFEST_PATH)).toBe(true);
    expect(isValidVaultPath("vault/manifest.bin")).toBe(true);
